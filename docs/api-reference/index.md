...
| `localASNumber` _[IntOrString](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.35/#intorstring-intstr-util)_ | LocalASNumber specifies a local AS number to present to the BGP peer.<br />When set, it is equivalent to LocalAS with ASNumber set and PrependLocalAS/PrependGlobalAS defaults.<br />Deprecated: Use LocalAS.ASNumber instead. This field will be removed in a future release. |  | Optional: \{\} <br /> |
